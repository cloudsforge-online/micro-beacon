 ...
 *
 * Here it matters once more than usual: below version 4 the partial unique index
 * `incidents_open_uniq` may not exist, and it is the only thing stopping an outage from opening
 * one incident per probe cycle. A service that could create it at boot is a service that could
 * start without it.
 *
...
try {
  ...
  log.info('migrations complete', {
    ...
    to: result.nowAt,
    applied: result.applied.map((applied) => `${applied.version}:${applied.name}`),
  })
