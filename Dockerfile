...
# build instead of the first request — and in this repository a type error is load-bearing, because
# the public projection's allowlist is asserted against its interface AT COMPILE TIME.
FROM deps AS build
...
# run time. There is no ENV line here on purpose — least of all BEACON_TOKEN, which is what gates
...
# then drifts — and this service exists to be the place health is stated once.
...
# below SCHEMA_VERSION the partial unique index `incidents_open_uniq` may not exist, and it is what
# stops an outage opening one incident per probe cycle; nor may
# `gate_decisions_indeterminate_never_promotes`, which is what stops an indeterminate gate
# evaluation being recorded as a promotion. A service that could create them at boot is a service
# that could start without them. `index.ts` asserts the schema version and refuses to serve below
# it.
